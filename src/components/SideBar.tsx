...
interface SidebarProps {
  onLinkClick: (link: string) => void; // Callback for link clicks
}
...
const Sidebar: React.FC<SidebarProps> = ({ onLinkClick }) => {
  ...
  const links = ['Calendar', 'Inbox', 'Notes', 'To-Do List', 'Settings'];

  return (
    <div className="w-[276px] h-[1000px] py-[40px] px-0 gap-[60px] rounded-tl-[20px] rounded-bl-[20px] bg-white fixed top-0 left-0 flex flex-col items-center">
      <Link to="/">
      ...
      <ul className="list-none p-0 m-0 flex flex-col items-center w-[276px] h-[380px]">
        {links.map((link) => (
            ...
            >
              <img
                ...
                className={`w-[24px] h-[24px] mr-[16px] ${
                  activeLink === link ? 'text-[#4F35F3]' : 'text-[#65676D]'
                }`}
                style={{
                  filter: activeLink === link ? 'invert(43%) sepia(84%) saturate(6638%) hue-rotate(192deg) brightness(95%) contrast(93%)' : 'none'
                }}
              />
              {link}
            </button>
